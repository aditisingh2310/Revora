  ...
  if (query.isLoading) {
    return (
      <div className="space-y-9">
        <div className="h-10 w-full max-w-64 animate-pulse rounded-lg bg-muted" />
        <div className="mt-2 h-5 w-full max-w-96 animate-pulse rounded-lg bg-muted" />
    ...
    <div className="space-y-6 sm:space-y-9">
      <div className="revora-rise flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div className="min-w-0">
          ...
          <h1 className="mt-2 font-serif text-3xl tracking-[-.045em] sm:text-4xl md:text-5xl">Your revenue network<br className="hidden sm:block" /><span className="text-muted-foreground/65">starts here.</span></h1>
        ...
        <a href="/onboarding" data-testid="link-add-channel" className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-xs font-semibold text-primary-foreground shadow-sm transition-transform hover:-translate-y-0.5 md:w-auto">+ Add a channel</a>
      </div>
      <SummaryCards summary={overview.summary} />
      <div className="grid gap-6 sm:gap-8 xl:grid-cols-[1.35fr_.65fr]">
        <section>
          <div className="mb-5">
                  ...
                  <div key={item.id} className="group flex gap-3 rounded-xl px-3 py-3 transition-colors hover:bg-muted/60" data-testid={`activity-item-${item.id}`}>
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
                        <p className="min-w-0 break-words text-xs font-semibold">{item.title}</p>
                        <span className="shrink-0 whitespace-nowrap font-mono text-[9px] text-muted-foreground">{fmtDate(item.createdAt)}</span>
                      </div>
